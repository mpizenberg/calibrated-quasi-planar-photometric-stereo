...
pub fn normal_integration(normals: &DMatrix<Vector3<f32>>) -> DMatrix<f32> {
    let (nrows, ncols) = normals.shape();
    let norms = normals.map(|v| v.norm());
    log::warn!("Min norm: {}", norms.min());
    log::warn!("Max norm: {}", norms.max());
    log::warn!("Mean norm: {}", norms.mean());
